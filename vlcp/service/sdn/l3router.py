...
class RouterUpdater(FlowUpdater):
            ...
            for subnet_weak in lgnetmap.subnets.dataset():
                try:
                    ...
                    if self._parent.ipaddress_inter_connect:
                        if hasattr(subnetobj, "external"):
                            try:
                                subnet_external_obj = walk(subnetobj.external.getkey())
                            except KeyError:
                                pass
                            else:
                                save(subnet_external_obj.getkey())
    ...
    def reset_initialkeys(self,keys,values):
        ...
        subnetexternalkeys = [k for k, v in zip(keys, values) if v is not None and v.isdeleted()
                              and v.isinstance(SubNetExternalInfo)]

        self._initialkeys = tuple(itertools.chain(self._original_keys,subnetkeys,
                                    routerportkeys,routerkeys,forwardinfokeys, subnetexternalkeys))
    ...
    def updateflow(self, connection, addvalues, removevalues, updatedvalues):
        ...
        try:
            ...
            subnet_to_external = dict((s, s.external) for s in allobjects if s.isinstance(SubNet)
                                      and hasattr(s, "external") and s.external in allobjects)
            ...
            if self._parent.enable_router_forward:
                ...
                if self._parent.ipaddress_inter_connect:
                    # we get external ip address from subnet external info
